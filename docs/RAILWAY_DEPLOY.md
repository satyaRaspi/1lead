# Railway Deployment — Truflux Website v1.0.24

This build is Railway-ready as a single service.

## Why v1.0.24 fixes the Railpack error
...
```bash
docker build -t truflux-website:1.0.24 .
docker run --rm -p 8000:8000 -e PORT=8000 truflux-website:1.0.24
```
...
## v1.0.24 note: Vite build fix

If Railway showed `sh: 1: vite: not found`, use v1.0.24 or later. The fix removes the frontend lock file and updates the Dockerfile to install Vite before running the production build.
